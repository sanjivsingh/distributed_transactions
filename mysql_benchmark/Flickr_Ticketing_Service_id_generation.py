...
class TicketingBookingShard:
    ...
    def __init__(
        self, host, user, password, port, database, table, index, numberofshards
    ):
        self.host = host
        self.user =user
    ...
    def connect(self):
        if self.connection is None:
            try:
                ...
                self.log.info(
                    f"shard[{self.index}] connected.....auto_increment_offset : {self.index} auto_increment_increment: {self.numberofshards}"
                )
    ...
    def insert(self, booking_name: str):
        ...
        with self.connection.cursor() as cursor:
            ...
            try:
                ...
                print(
                    f"shard[{self.index}]  booking_name {booking_id}: {booking_name} insert successfully."
                )
                self.insert_count += 1
            except Exception as err:
                print(
                    f"shard[{self.index}] Error insert booking_name '{booking_name}': {err}"
                )
                self.connection.rollback()
